        ...
        for k, f in enumerate(self.feature_maps_wh):
            ...
            for i in range(grid_h):
                for j in range(grid_w):
                    f_k_h = self.img_wh[1] / float(self.steps[k][1])
                    f_k_w = self.img_wh[0] / float(self.steps[k][0])
                    # unit center x,y
                    cx = (j + 0.5) / float(f_k_w)
                    cy = (i + 0.5) / float(f_k_h)

                    # aspect_ratio: 1
                    # rel size: min_size
                    s_k_h = self.min_sizes[k] / float(self.img_wh[1])
                    s_k_w = self.min_sizes[k] / float(self.img_wh[0])
                    ...
                    if self.use_max_sizes:
                        s_k_prime_w = sqrt(
                            s_k_w * (self.max_sizes[k] / float(self.img_wh[0])))
                        s_k_prime_h = sqrt(
                            s_k_h * (self.max_sizes[k] / float(self.img_wh[1])))
