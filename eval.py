        ...
        with torch.no_grad():
            ...
            for k in range(boxes.size(0)):
                ...
                for j in range(1, num_classes):
                    ...
                    inds = np.where(scores_[:, j] > 0.0001)[0]
                    if len(inds) == 0:
                    ...
                    t_nms_e = time.time()
                    print('true nms time  {:.3f}s'.format(t_nms_e-t_nms_s))
                    keep = keep[:50]
                    c_dets = c_dets[keep, :]
                    all_boxes[j][i] = c_dets

                #img = imgs[k].cpu().numpy().transpose(1,2,0) + cfg.TRAIN.BGR_MEAN
                #img = img.astype("uint8")
                #for box in all_boxes[j][i]:
                #    draw_img = draw_rects(img, [box,j], VOC_CLASSES)
                #img = img *255
                # bgr_means = cfg.TRAIN.BGR_MEAN
                # cv2.imwrite(str(i)+".jpg", img.astype("uint8"))
            t3 = time.time()
            detect_time = t2 - t1
            nms_time = t3 - t2
            forward_time = t4 - t1
