...
def _expand(image, boxes, fill, p):
    ...
    for _ in range(50):
        ...
        expand_image[:, :] = [fill[0]*255, fill[1]*255, fill[2]*255] 
...
def normalize(im, mean=[0, 0, 0], std=[1, 1, 1], rgb=True):
    if rgb:
        r, g, b = cv2.split(im)
    else:
        b, g, r = cv2.split(im)
    norm_im = cv2.merge([(r - 255*mean[0]) / 255,(g - 255*mean[1]) / 255,(b - 255*mean[2]) / 255])
    return norm_im

def preproc_for_test(image, resize_wh, mean, std):
    interp_methods = [
...
class preproc(object):
    def __init__(self, resize_wh, means, std, p=0.6):
        self.means = means
        self.std = std
    ...
    def __call__(self, image, targets):
        ...
        if len(boxes) == 0:
            ...
            image = preproc_for_test(image, self.resize_wh, self.means, self.std)
        ...
        image_t, boxes = _expand(image_t, boxes, self.means, 0.6)
        image_t, boxes = _mirror(image_t, boxes)
        #image_t, boxes = _mirror(image, boxes)

        height, width, _ = image_t.shape
        image_t = preproc_for_test(image_t, self.resize_wh, self.means, self.std)
        boxes = boxes.copy()
        boxes[:, 0::2] /= width
        boxes[:, 1::2] /= height
        ...
        if len(boxes_t) == 0:
            image = preproc_for_test(image_o, self.resize_wh, self.means, self.std)
...
class BaseTransform(object):
    ...
    def __init__(self, resize_wh, means, std, swap=(2, 0, 1)):
        self.means = means
        self.resize_wh = resize_wh
        self.std = std
        self.swap = swap
    ...
    def __call__(self, img, target=None):
        ...
        img = bgr2rgb(img)
        img = normalize(img, self.means, self.std)
        #img -= self.means
