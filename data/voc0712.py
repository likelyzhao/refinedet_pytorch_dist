...
class VOCDetection(data.Dataset):
    ...
    def evaluate_detections_dist(self, all_boxes, output_dir=None):
        """
        ...
        """
        self._write_voc_results_file_dist(all_boxes,output_dir)
        return self._do_python_eval_dist(output_dir)
    ...
    def _get_voc_results_file_template_dist(self,output_dir):
        filename = 'comp3_det_test' + '_{:s}.txt'
        filedir = os.path.join(output_dir, 'results', 'VOC' + self._year,
                               'Main')
        if not os.path.exists(filedir):
            os.makedirs(filedir)
        path = os.path.join(filedir, filename)
    ...
    def _write_voc_results_file_dist(self, all_boxes,output_dir):
        for cls_ind, cls in enumerate(VOC_CLASSES):
            if cls == '__background__':
                continue
            print('Writing {} VOC results file'.format(cls))
            filename = self._get_voc_results_file_template_dist(output_dir).format(cls)
            # print(filename)
    ...
    def _do_python_eval_dist(self, output_dir='output'):
